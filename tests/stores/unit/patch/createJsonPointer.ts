...
import createJsonPointer, { navigate } from '../../../src/patch/createJsonPointer';

registerSuite({
	name: 'JsonPointer',
	'Should work with simple path.'(this: any) {
		const target = { prop1: 1 };
		const path = createJsonPointer('prop1');
	...
	'Should work with multiple paths.'(this: any) {
		...
		const path = createJsonPointer('prop1', 'prop2', 'prop3');
		const result = navigate(path, target);
	...
	'Should create new path using push.'(this: any) {
		const target = { prop1: { prop2: { prop3: 1 } } };
		const path = createJsonPointer('prop1', 'prop2').push('prop3');
		const result = navigate(path, target);
	...
	'Should create new path using pop.'(this: any) {
		const target = { prop1: { prop2: { prop3: 1 } } };
		const path = createJsonPointer('prop1', 'prop2', 'prop3').pop();
		const result = navigate(path, target);
	...
	'Should have a toString that describes the path.'(this: any) {
		const path = createJsonPointer('a', 'b', 'c').toString();
