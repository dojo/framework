...
registerSuite('support/virtualDom', {
	'assignChildProperties()': {
		...
		'does not resolve'() {
			...
			assert.throws(
				...
				TypeError,
				'Index of "0" is not resolving to a valid target'
			);
		}
	},
	...
	'assignChildPropertiesByKey()': {
		...
		'does not resolve - string'() {
			...
			assert.throws(
				...
				TypeError,
				'Key of "a" is not resolving to a valid target'
			);
		},
		...
		'does not resolve - object'() {
			...
			assert.throws(
				...
				TypeError,
				'Key of "{}" is not resolving to a valid target'
			);
		},
		...
		'duplicate key'() {
			...
			assertRender(
				actual,
				v('div', {}, [v('a', { target: '_blank', key: 'a' }), v('a', { key: 'a', href: '#link' })])
			);
	...
	'assignProperties()': {
		...
		'with class function'() {
			...
			assertRender(
				actual,
				v('div', { styles: { color: 'blue' }, classes: ['testClass'] }, [null, v('a', { href: '#link' })])
			);
		}
	},
	...
	'replaceChild()': {
		...
		'string index resolving to a non child node throws'() {
			...
			assert.throws(
				...
				TypeError,
				'Index of "0,0,0" is not resolving to a valid target'
			);
		},
		...
		'string index resolve to an earlier non child node throws'() {
			...
			assert.throws(
				...
				TypeError,
				'Index of "3,0,0" is not resolving to a valid target'
			);
		}
	},
	...
	'replaceChildByKey()': {
		...
		'nested child'() {
			...
			assertRender(
				actual,
				v('div', { key: 'a' }, [v('div', {}, [v('div', { key: 'b' })]), v('a', { href: '#link' })])
			);
	...
	'findKey()': {
		...
		'key not found with children'() {
			assert.isUndefined(
				findKey(v('div', [v('span', { key: 'bar' }), 'foo', null]), 'foo'),
				'should not find key'
			);
		},
		...
		'key is in child'() {
			assertRender(
				findKey(v('div', { key: 'bar' }, [v('span', { key: 'foo' }), 'foo', null]), 'foo')!,
				v('span', { key: 'foo' }),
				'should find child node'
			);
			assertRender(
				findKey(
					v('div', { key: 'bar' }, [v('span', { key: 'foo' }), v('span', { key: 'baz' }), 'foo', null]),
					'baz'
				)!,
				v('span', { key: 'baz' }),
				'should find child node'
		...
		'key is object'() {
			const obj = {};
			assertRender(
				findKey(v('div', { key: 'bar' }, [v('span', { key: obj } as any), 'foo', null]), obj)!,
				v('span', { key: obj } as any),
				'should find child node'
			);
		},
		...
		'value is WNode'() {
			const vnode = w<any>(
				...
				[w<any>('sub-widget', { key: 'foo', onClick() {} }), w<any>('sub-widget', { key: 'bar', onClick() {} })]
			);
		...
		'duplicate string keys warn'() {
			...
			const fixture = v('div', { key: 'foo' }, [
				v('span', { key: 'parent1' }, [v('i', { key: 'icon', id: 'i1' })]),
				v('span', { key: 'parent2' }, [v('i', { key: 'icon', id: 'i2' })])
			]);
		...
		'duplicate object keys warn'() {
			...
			const fixture = v('div', { key: 'foo' }, [
				v('span', { key: 'parent1' }, [v('i', { key, id: 'i1' })]),
				v('span', { key: 'parent2' }, [v('i', { key, id: 'i2' })])
			]);
