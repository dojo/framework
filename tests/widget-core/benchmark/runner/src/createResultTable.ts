...
fs
	...
	.forEach((name) => {
		...
		if (!frameworkMap.has(data.framework)) {
			console.log(
				'WARN: No entry in commons.ts for ' + data.framework + '. Data will not appear in result table.'
			);
		} else {
...
let cpuBenchmarks = benchmarks.filter(
	(benchmark) => benchmark.type === BenchmarkType.CPU || benchmark.type === BenchmarkType.STARTUP
);
...
let generateBenchData = (
	benchmarks: Array<Benchmark>,
	frameworkPredicate: FrameworkPredicate,
...
function frameworkPredicateKeyed(keyed: boolean): FrameworkPredicate {
	return (framework: FrameworkData) => {
		return framework.keyed === keyed;
	};
}
